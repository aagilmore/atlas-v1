...
## 0022 – Introduce Observation Inbox

### Purpose

Introduce a dedicated location for newly generated Atlas observations before they are imported into the permanent repository.

### Design Intent

Separate transient observation files from the permanent historical record.

### Key Decisions

- Added an `inbox/` directory to the repository.
- Defined the Inbox as the staging area for observations generated by ChatGPT.
- Kept the Inbox independent from the Observation Repository.

### Outcome

Atlas now has a defined entry point for new observations, completing another foundational element of the product architecture.

## 0023 – Discover Pending Observations

### Purpose

Introduce the Observation Inbox service responsible for discovering new observations awaiting import.

### Design Intent

Separate inbox management from repository management.

### Key Decisions

- Added `observation_inbox.py`.
- Introduced `get_pending_observations()`.
- Limited the service to observation discovery only.

### Outcome

Atlas can now identify pending observation files without importing them.

## 0024 – Import Observation

### Purpose

Implement the complete observation import workflow from the Atlas Inbox into the Observation Repository.

### Design Intent

Enable Atlas to ingest observations generated by ChatGPT while preserving the separation between incoming and permanent data.

### Key Decisions

- Added an Observation Importer service.
- Imported observations are copied from the Inbox into the Observation Repository.
- The importer discovers pending observations, allows the user to select one, and imports it.
- Dashboard behavior remains unchanged because it already reads from the Observation Repository.

### Outcome

Atlas now supports a complete end-to-end workflow for importing observations and viewing them on the dashboard.